<div>
    ...
    <div class="form-group">
        <label for="inputRestaurantDay">{{ __('reservation.restaurant.form.day.label') }}</label>
        <select id="inputRestaurantDay" wire:model="day" class="form-control @error('day') is-invalid @enderror">
            <option>{{ __('reservation.restaurant.form.day.option.default') }}</option>
    ...
    @if($dataValid)
        <button wire:click="confirmTime" class="btn btn-primary">{{ __('reservation.restaurant.button.confirm-reservation') }}</button>
    @endif
</div>
